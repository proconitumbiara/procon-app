 ...
 *
 * Modelo de dados:
 *  - Banco: TIMESTAMP WITHOUT TIME ZONE armazenado em UTC (Node.js/PostgreSQL em UTC)
 *  - Brasil: permanentemente UTC-3 desde abolição do horário de verão em 2019
 *  - Filtros de data: strings YYYY-MM-DD representando datas no calendário de Brasília
 */

/**
 * Converte uma string "YYYY-MM-DD" (dia no calendário de Brasília/São Paulo)
 * para o instante UTC equivalente ao início desse dia (00:00:00 BRT = 03:00:00 UTC).
 * Usar para o limite inferior de filtros de data.
 */
export function saoPauloDateStringToUTCStart(dateStr: string): Date {
  return new Date(`${dateStr}T00:00:00.000-03:00`);
}

/**
 * Converte uma string "YYYY-MM-DD" (dia no calendário de Brasília/São Paulo)
 * para o instante UTC equivalente ao fim desse dia (23:59:59.999 BRT = 02:59:59.999 UTC+1dia).
 * Usar para o limite superior de filtros de data.
 */
export function saoPauloDateStringToUTCEnd(dateStr: string): Date {
  return new Date(`${dateStr}T23:59:59.999-03:00`);
}

/**
 * Retorna o ano, mês e último dia do mês atual no fuso de São Paulo.
 * Usar para calcular os defaults de períodos nos filtros de data do servidor.
 */
